...
export default function AdminPanel() {
  ...
  const refreshUserRequests=()=>{
    try{ setUserRequests(JSON.parse(localStorage.getItem("driverent_pending_requests")||"[]")); }catch{}
    // Load from Supabase
...
function UserAccountsPage({userRequests, setUserRequests, refreshUserRequests}) {
  ...
  const handleApprove=(req)=>{
    const pending=JSON.parse(localStorage.getItem("driverent_pending_requests")||"[]");
    const updatedPending=pending.map(r=>r.id===req.id?{...r,status:"approved",reviewedAt:new Date().toISOString()}:r);
    localStorage.setItem("driverent_pending_requests",JSON.stringify(updatedPending));
    const approved=getApprovedUsers();
    const newUser={id:req.id,username:req.username,password:req.password,name:req.name||req.username,phone:req.phone,avatar:req.avatar||`https://i.pravatar.cc/80?u=${req.username}`,provider:"manual",joinDate:req.requestDate,lastLogin:null,approvedAt:new Date().toISOString()};
    if(!approved.find(u=>u.id===newUser.id)){
      localStorage.setItem("driverent_approved_users",JSON.stringify([...approved,newUser]));
    }
    setUserRequests(updatedPending);
    setConfirmModal(null); setViewModal(null);
  };

  const handleReject=(req)=>{
    const pending=JSON.parse(localStorage.getItem("driverent_pending_requests")||"[]");
    const updatedPending=pending.map(r=>r.id===req.id?{...r,status:"rejected",reviewedAt:new Date().toISOString()}:r);
    localStorage.setItem("driverent_pending_requests",JSON.stringify(updatedPending));
    setUserRequests(updatedPending);
    setConfirmModal(null); setViewModal(null);
  };
  ...
  const handleDelete=(req)=>{
    const pending=JSON.parse(localStorage.getItem("driverent_pending_requests")||"[]");
    const updatedPending=pending.filter(r=>r.id!==req.id);
    localStorage.setItem("driverent_pending_requests",JSON.stringify(updatedPending));
    setUserRequests(updatedPending);
  };
        ...
        <div style={{position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,.78)",backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>{if(e.target===e.currentTarget)setViewModal(null);}}>
          <div style={{background:"#0F0F1A",border:`1px solid ${statusColor[viewModal.status]}44`,borderRadius:22,padding:28,width:"100%",maxWidth:420,animation:"fadeIn .25s ease",position:"relative"}}>
            ...
            {[
              ...
              {icon:"🔐",label:"كلمة السر",val:viewModal.password,mono:true},
